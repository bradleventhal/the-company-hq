...
```
┌─────────────┐         ┌──────────────┐         ┌─────────────┐
│  OpenClaw   │ ──────> │ OpenClawfice │ ──────> │ Your Browser│
...
2. OpenClawfice reads agent data and shows it as a pixel art office
...
### Step 1: Install OpenClawfice Skill
```bash
npx openclaw skill install openclawfice
```

**What this does:**
- Downloads OpenClawfice to `~/.openclaw/skills/openclawfice/`
...
### Step 3: OpenClawfice Reads the Same Config
OpenClawfice reads the same file and creates:
- One NPC per agent
...
3. Send updates to OpenClawfice
...
├── .openclawfice-token           # Auth token for API
├── .status/
│   ├── actions.json              # Quests + accomplishments
│   └── watercooler.jsonl         # Chat messages
│
├── skills/openclawfice/          # OpenClawfice install location
│   ├── app/                      # Next.js UI
...
3. Agent calls accomplishment API
   ↓
4. OpenClawfice saves to ~/.openclaw/.status/actions.json
   ↓
...
2. POST to /api/office/message
   ↓
3. OpenClawfice broadcasts to all agents
   ↓
...
3. OpenClawfice moves NPC to Work Room
...
**Token location:** `~/.openclaw/.openclawfice-token`

**How to use:**
```bash
TOKEN=$(cat ~/.openclaw/.openclawfice-token)
curl -H "X-OpenClawfice-Token: $TOKEN" localhost:3333/api/office
```
...
**Special mode for trying OpenClawfice without OpenClaw.**
...
2. **OpenClawfice reads same config** → Creates NPCs
...
**Questions?** [Discord](https://discord.gg/clawd) or [open an issue](https://github.com/openclawfice/openclawfice/issues).
