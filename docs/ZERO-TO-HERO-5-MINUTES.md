# Zero to Hero: Your First 5 Minutes with OpenClawfice
...
## Step 2: Install OpenClawfice (2 minutes)

### Option A: If you have OpenClaw already

```bash
# Install the skill
openclaw skill install openclawfice
...
**Yes!** Edit `openclawfice.config.json`:
...
**No.** OpenClawfice just reads from:
...
**Not yet.** OpenClawfice is OpenClaw-specific because it:
...
### Keep It Open
Run OpenClawfice in a separate browser window. Pin it to a second monitor. Glance over to see what's happening.
...
- **Docs**: All guides in `openclawfice/docs/`
